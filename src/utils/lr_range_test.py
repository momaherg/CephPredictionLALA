    ...
    try:
        with tqdm(total=num_iterations) as pbar:
            for batch in train_loader:
                ...
                # CephalometricDataset returns a dict with 'image' and 'landmarks' keys
                if isinstance(batch, dict) and 'image' in batch and 'landmarks' in batch:
                    inputs = batch['image'].to(device)
                    targets = {
                        'heatmaps': None,  # This will be generated by the model or loss function
                        'coords': batch['landmarks'].to(device)
                    }
                elif isinstance(batch, (list, tuple)) and len(batch) == 2:
                    # Standard format: (inputs, targets)
                    inputs, target_data = batch
                    ...
                    if isinstance(target_data, torch.Tensor):
                        ...
                        targets = {k: v.to(device) if isinstance(v, torch.Tensor) else v for k, v in target_data.items()}
                    else:
                        targets = target_data  # Keep as is if not tensor or dict
                else:
                    # Unknown format, try to use the batch directly
                    inputs = batch
                    targets = None
                ...
                if isinstance(outputs, dict) and isinstance(targets, dict):
                    ...
                    try:
                        ...
                        # If the above format doesn't work, try a more generic approach
